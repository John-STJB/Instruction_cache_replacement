...
# Sanity check
if [ ! -f ./branch/${BRANCH}.bpred ] || [ ! -f ./prefetcher/${L1D_PREFETCHER}.l1d_pref ] || [ ! -f ./prefetcher/${L2C_PREFETCHER}.l2c_pref ] || [ ! -f ./replacement/${L1I_REPLACEMENT}.l1i_repl ] || [ ! -f ./replacement/${LLC_REPLACEMENT}.llc_repl ]; then
	...
	echo
	echo "${BOLD}Possible L1I Replacement: ${NORMAL}"
	LIST=$(ls replacement/*.l1i_repl | cut -d '/' -f2 | cut -d '.' -f1)
	p=$( embed_newline $LIST )
	echo "$p"
...
echo "Cores: ${NUM_CORE}"
BINARY_NAME="${BRANCH}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${L1I_REPLACEMENT}-${LLC_REPLACEMENT}-${NUM_CORE}core"
...
cp replacement/l1i.l1i_repl replacement/l1i_replacement.cc
