...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
            ...
            way = l1i_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
        }
        else
        ...
        if(cache_type==IS_L1I)   
            l1i_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr,block[set], MSHR.entry[mshr_index].ip, block[set][way].full_addr, MSHR.entry[mshr_index].type, 0);
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_L1D) { // RFO miss
                ...
                if (cache_type == IS_LLC) {
                    ...
                    way = l1i_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type);
                }
                else
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            if (way >= 0) { // read hit
                ...
                if (cache_type == IS_LLC) {
                    ...
                    l1i_update_replacement_state(read_cpu, set, way, block[set][way].full_addr,block[set], RQ.entry[index].ip, 0, RQ.entry[index].type, 1);
...
{   
    ...
    if (PQ.occupancy && (RQ.occupancy == 0) && (cache_type != IS_L1I))
        handle_prefetch();
}
