...
class CACHE : public MEMORY {
  public:
    ...
    void add_mshr(PACKET *packet),
         ...
         l1i_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr,BLOCK *current_set,uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit),
    ...
    uint32_t get_set(uint64_t address),
             ...
             l1i_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set,  BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),
